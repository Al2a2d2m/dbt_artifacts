...
{% macro netezza__get_invocations_dml_sql() -%}
    {% set invocation_values  %}
        (
        '{{ invocation_id }}', {# command_invocation_id #}
        '{{ dbt_version }}', {# dbt_version #}
        ...
        {% if var('env_vars', none) %}
            ...
            {% for env_variable in var('env_vars') %}
                {% do env_vars_dict.update({env_variable: (env_var(env_variable, ''))}) %}
            {% endfor %}
            '{{ tojson(env_vars_dict) |replace("'", '"') | replace("\\", "\\\\") | replace("'", "\\'") | replace('"', '\\"') }}', {# env_vars #}
        ...
        {% if var('dbt_vars', none) %}
            ...
            {% for dbt_var in var('dbt_vars') %}
                {% do dbt_vars_dict.update({dbt_var: (var(dbt_var, ''))}) %}
            {% endfor %}
            '{{ tojson(dbt_vars_dict) |replace("'", '"') | replace("\\", "\\\\") | replace("'", "\\'") | replace('"', '\\"') }}', {# env_vars #}
        {% else %}
            null, {# dbt_vars #}
        {% endif %}
        ...
        '{{ tojson(invocation_args_dict) |replace("'", '"') | replace("\\", "\\\\") | replace("'", "\\'") | replace('"', '\\"') }}', {# invocation_args #}

        {% set metadata_env = {} %}
        {% for key, value in dbt_metadata_envs.items() %}
            {% do metadata_env.update({key: value}) %}
        {% endfor %}
        '{{ tojson(metadata_env) |replace("'", '"') | replace("\\", "\\\\") | replace("'", "\\'") | replace('"', '\\"') }}' {# dbt_custom_envs #}

        )
    {% endset %}
    {{ return([invocation_values]) }}
