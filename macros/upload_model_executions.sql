...
{% macro netezza__get_model_executions_dml_sql(models) -%}
    {% if models != [] %}
        {% set model_execution_values = [] %}
        ...
        {% for model in models -%}
            {%- set data_row -%}
                ...
                {{ model.execution_time }}, {# total_node_runtime #}
                null,  {# Only available in Snowflake & BigQuery #}
                '{{ model.node.config.materialized }}', {# materialization #}
                '{{ model.node.schema }}', {# schema #}
                '{{ model.node.name }}', {# name #}
                '{{ model.node.alias }}', {# alias #}
            ...
            {% endset %}
        {%- do model_execution_values.append(data_row) -%}
            
        {%- endfor %}
       
        {{ return(model_execution_values) }}
    {% else %}
        {{ return([]) }}
    {% endif %}
{% endmacro -%}