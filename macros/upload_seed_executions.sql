...
{% macro netezza__get_seed_executions_dml_sql(seeds) -%}
    {% if seeds != [] %}
        {% set seed_execution_values = [] %}
        {% for model in seeds -%}
        {%- set data_row -%}
            (
                ...
                {{ model.execution_time }}, {# total_node_runtime #}
                null, -- rows_affected TO DO {# Databricks #}
                '{{ model.node.config.materialized }}', {# materialization #}
                '{{ model.node.schema }}', {# schema #}
                '{{ model.node.name }}', {# name #}
                '{{ model.node.alias }}', {# alias #}
        ...
        {% endset %}
    {%- do seed_execution_values.append(data_row) -%}
    {%- endfor %}
        
        {{ return(seed_execution_values) }}
    {% else %}
        {{ return([]) }}
    {% endif %}
{% endmacro -%}