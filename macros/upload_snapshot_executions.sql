...
{% macro netezza__get_sources_dml_sql__get_snapshot_executions_dml_sql(snapshots) -%}
    {% if snapshots != [] %}
        {% set snapshot_execution_values =[]%}
        {% for model in snapshots -%}
            {%- set data_row -%}
            (
                ...
                {{ model.execution_time }}, {# total_node_runtime #}
                null, -- rows_affected TODO {# Databricks #}
                '{{ model.node.config.materialized }}', {# materialization #}
                '{{ model.node.schema }}', {# schema #}
                '{{ model.node.name }}', {# name #}
                '{{ model.node.alias }}', {# alias #}
            ...
            {% endset %}
        {%- do snapshot_execution_values.append(data_row) -%}

        {%- endfor %}
        {{ return(snapshot_execution_values) }}
    {% else %}
        {{ return([]) }}
    {% endif %}
{% endmacro -%}
